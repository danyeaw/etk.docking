...
class _DockGroupTab(object):
    ...
    __slots__ = [
        "item",  # DockItem associated with this tab
        ...
        "last_focused",
    ]  # timestamp set last time a tab was focused

    def __contains__(self, pos):
        return rect_contains(self.area, *pos)

    def __str__(self):
        return "<%s object at 0x%s with label '%s' on %s>" % (
...
class DockGroup(Gtk.Container):
    ...
    __gsignals__ = {
        ...
        "item-selected": (GObject.SignalFlags.RUN_LAST, None, (GObject.TYPE_OBJECT,)),
    }
    ...
    def do_motion_notify_event(self, event):
        ...
        if event.window is self.window:
            ...
            if (
                ...
                tab = self.get_tab_at_pos(
                    self.dragcontext.source_x, self.dragcontext.source_y
                )
                ...
                if self.dragcontext.dragged_object:
                    self.drag_begin(
                        ...
                        Gdk.DragAction.MOVE,
                        self.dragcontext.source_button,
                        event,
                    )
    ...
    def do_forall(self, include_internals, callback, callback_data=None):
        if callback_data:
            ...
            if include_internals:
                ...
                callback(self._list_button)
                callback(self._min_button)
                callback(self._max_button)
    ...
    def _insert_item(self, item, position=None, visible_position=None):
        ...
        tab.item_title_handler = tab.item.connect(
            "notify::title", self._on_item_title_changed, tab
        )
