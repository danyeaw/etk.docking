...
class DockLayout(GObject.GObject):
    ...
    def get_main_frames(self):
        ...
        return (
            ...
            if not (
                isinstance(f.get_parent(), Gtk.Window)
    ...
    def _get_signals(self, widget):
        ...
        if isinstance(widget, DockPaned):
            ...
            signals = (("add", self.on_widget_add), ("remove", self.on_widget_remove))
        else:
            signals = ()
    ...
    def add_signal_handlers(self, widget):
        ...
        if not drag_dest:
            widget.drag_dest_set(
                Gtk.DestDefaults.MOTION, [DRAG_TARGET_ITEM_LIST], Gdk.DragAction.MOVE
            )
    ...
    def do_item_selected(self, group, item):
        ...
        if not (
            group is self._focused_group
            and group.get_tab_state() != Gtk.StateType.PRELIGHT
        ):
    ...
    def on_widget_drag_data_received(
        ...
        """
        self.log.debug(
            "drag_data_received %s, %s, %s, %s, %s, %s"
...
def _window_delete_handler(window, event):
    list(map(lambda i: i.close(), [i for i in flatten(window) if isinstance(i, DockItem)]))
    return False
...
def dock_group_highlight(self):
    if not hasattr(self, "_expose_event_id"):
        ...
        self._expose_event_id = self.connect_after(
            "expose-event", dock_group_expose_highlight
        )
    self.queue_draw()
...
def dock_group_drag_motion(self, context, x, y, timestamp):
    self.log.debug(
        "dock_group_drag_motion: %s, %s, %s, %s" % (context, x, y, timestamp)
    )
    ...
    def dock_group_drag_data_received(selection_data, info):
        self.log.debug(
            "%s, %s, %s, %s, %s, %s" % (context, x, y, selection_data, info, timestamp)
        )
        source = context.get_source_widget()
        assert source
...
def dock_paned_highlight(self):
    if not hasattr(self, "_expose_event_id"):
        self.log.debug("attaching expose event")
        self._expose_event_id = self.connect_after(
            "expose-event", dock_paned_expose_highlight
        )
    self.queue_draw()
...
def dock_paned_drag_motion(self, context, x, y, timestamp):
    self.log.debug(
        "dock_paned_drag_motion: %s, %s, %s, %s" % (context, x, y, timestamp)
    )
